...
FROM alpine:3.16.1@sha256:7580ece7963bfa863801466c0a488f11c86f85d9988051a9f9c68cb27f6b7872 AS release-dynamic
...
ARG TARGETPLATFORM
WORKDIR /usr/src
RUN wget https://www.libssh.org/files/0.9/libssh-0.9.6.tar.xz -O libssh-0.9.6.tar.xz
RUN tar -xa --strip-components=1 -f libssh-0.9.6.tar.xz
RUN \
    $(setvars ${TARGETPLATFORM}) && \
    cmake \
        -B build \
